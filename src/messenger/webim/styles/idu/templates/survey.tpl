...
<form id="surveyForm" name="surveyForm" method="post" action="${webimroot}/client.php">
...
${endif:showmessage}
			<tr>
				<td colspan="2" align="right">
					<table cellspacing="0" cellpadding="0" border="0">
					<tr>
					<td><a href="#" class="but" id="sndmessagelnk">${msg:presurvey.submit}</a></td>
					</tr>
					</table>
				</td>
			</tr>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#sndmessagelnk').click(function(){ $('#surveyForm').submit() });
        $('#surveyForm').submit(function() {
            var is_valid = true;
            var name = $.trim($('input[name="name"]').val());
            var message = $.trim($('textarea[name="message"]').val());
            if( name == 'Visitante' || name == '') {
            ...
            if( message == '') {
                alert('Por favor ingrese su pregunta inicial')
                is_valid = false;
            }
